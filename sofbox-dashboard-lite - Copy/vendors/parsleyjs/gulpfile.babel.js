...
function buildDoc(done) {
  ...
  docco.document({
    ...
    args: sources
      ...
      .pipe($.replace('<div id="jump_page">', '<div id="jump_page"><a class="source" href="ex.html"><<< back to documentation</a>'))
...
function copyI18n(done) {
  gulp.src(['src/i18n/*.js'])
    .pipe($.replace("import Parsley from 'sley';", "// Load this after Parsley"))  // Quick hack
    .pipe($.replace("import Parsley from 'sley/main';", ""))  // en uses special import
